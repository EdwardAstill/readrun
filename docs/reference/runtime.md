...
content folder
  ...
  -> serve page assets to one Electron window
...
the Electron process and leaves the server running. The page inside the window is
...
`rr web` selects the platform browser opener instead of Electron and also leaves
